    ...
    :QLabel(parent),mask(parent)
{
    ...
    connect(timer,&QTimer::timeout,[=](){
        if(coldTime<maxColdTime[id])
        {
            ...
            coldTime+=TICKTIME;
            mask.setGeometry(CARD_X*(id-1),CARD_Y*coldTime/maxColdTime[id],CARD_X,CARD_Y-CARD_Y*coldTime/maxColdTime[id]);
        }
    });
}
...
void Card::mousePressEvent(QMouseEvent *ev)
{
    if(ev->button()==Qt::LeftButton)
    {
        ...
        if(grandParent->emptyHand()&&grandParent->enough(plantCost[id])&&coldTime>=maxColdTime[id])
