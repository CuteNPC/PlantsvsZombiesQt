...
Car::Car(GameWidget* ptr, int y):                   //构造小推车
    QLabel((QWidget*)ptr), timer(new QTimer()), ani(new QPropertyAnimation(this, "geometry", (QWidget*)ptr))
{
    ...
    connect(ani, SIGNAL(finished()), this, SLOT(try_to_set_null()));    //这两行表示：小推车走完全程，命令父对象将其移除（把它的指针置空）
...
void Car::try_to_set_null(){            //命令父对象把它的指针置空，以免出现野指针，之后生成的僵尸不再与之相关联
