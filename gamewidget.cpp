...
GameWidget::GameWidget(QWidget *parent,int stage_):
    QWidget(parent), mainthread(new Mainthread(this)), zomthread(new Zombiethread(this,stage_)),
    Suntimer(new QTimer())/*, refresh_timer(new QTimer())*/, zom_mutex(new QMutex()), sun_mutex(new QMutex())
{
    //把返回按钮与结束游戏的功能连接起来
    connect(mainthread, &Mainthread::finished, [=](){((MainStackedWidget*)(parentWidget()))->gameEnd(result);});
    //初始化
    stage=stage_;
    sunshine=150;
    plantInHand=0;
...
void GameWidget::createPlant(int x_,int y_,int id_)//根据id创造植物
{
    switch(id_)
    {
    case 1:plants[y_][x_]=new PeaShooter(this,y_,x_);break;
    case 2:plants[y_][x_]=new SunFlower(this,y_,x_);break;
    case 3:plants[y_][x_]=new Nut(this,y_,x_);break;
    case 4:plants[y_][x_]=new ColdPeaShooter(this,y_,x_);break;
    case 5:plants[y_][x_]=new TwoPeaShooter(this,y_,x_);break;
    }
}
