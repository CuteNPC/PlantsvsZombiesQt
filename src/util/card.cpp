...
// 卡槽类 实现
Card::Card(QWidget *parent, int id_) : QLabel(parent), mask(parent)
{
    ...
    connect(timer, &QTimer::timeout,
            [=]()
            {
                if (coldTime < maxColdTime[id])
                {
                    ...
                    coldTime += TICKTIME;
                    mask.setGeometry(CARD_X * (id - 1), CARD_Y * coldTime / maxColdTime[id], CARD_X,
...
// 设置冷却时间，用于清零
void Card::setColdTime(int x) { coldTime = x; }
