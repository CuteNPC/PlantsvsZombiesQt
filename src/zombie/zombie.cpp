...
once_movie::once_movie(QString s)
    : QMovie(s)
...
Zombie::Zombie(int loc,
    ...
    , eat_movs(new once_movie* [state_number] { nullptr })
    , state_now(0)
    , eating(new QTimer())
    , debuff(new QTimer())
    , ani(new QPropertyAnimation(this, "geometry", (QWidget*) ptr))
{
...
{ //主要操作函数
    ...
    if (floor(fmin(full_strength - strength, full_strength) * state_number / full_strength)
        > state_now) { //根据血量更新状态
        //mutex->lock();
        state_now = floor(fmin(full_strength - strength, full_strength) * state_number
                          / full_strength);
        emit change_state();
    }
    if (state_now >= state_number)
