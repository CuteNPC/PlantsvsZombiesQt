...
once_movie::once_movie(QString s) : QMovie(s)
{ // 自定义的movie，每播放一轮发出一次once_finished信号
...
Zombie::Zombie(int loc, double fs, double a, double sp, float es, int st, QString *movie_names,
    ...
    : QLabel((QWidget *)ptr), strength(fs), full_strength(fs), attack(a), speed(sp),
      state_number(st), parent(ptr), mutex(new QMutex()),
      movs(new once_movie *[state_number] { nullptr }),
      eat_movs(new once_movie *[state_number] { nullptr }), state_now(0), eating(new QTimer()),
      debuff(new QTimer()), ani(new QPropertyAnimation(this, "geometry", (QWidget *)ptr))
{
    ...
    if (parent->cars[loc] != nullptr)
    { // 生成时若该行还有小推车，将该僵尸与小推车关联
        connect(parent->cars[loc], SIGNAL(kill(int)), this, SLOT(die_now(int))); // 小推车杀死僵尸
        connect(this, SIGNAL(activate_car()), parent->cars[loc],
                SLOT(activate())); // 僵尸激活小推车
    }
    movie()->start();
    show();
    ani->start();
...
void Zombie::stop_to_eat(int a, int b)
{ // 僵尸遇到植物，停下来吃东西
    if_stop = 1;
    ani->pause();
    setMovie(eat_movs[state_now]);
...
void Zombie::attack_enemy()
{ // 僵尸进食状态下周期性执行，对面前的植物造成伤害
    enemy->hurt();
}
...
void Zombie::recover_from_debuff()
{ // 减速状态持续时间已到，解除减速
...
{ // 主要操作函数
    ...
    if (floor(fmin(full_strength - strength, full_strength) * state_number / full_strength) >
        state_now)
    { // 根据血量更新状态
        // mutex->lock();
        state_now =
            floor(fmin(full_strength - strength, full_strength) * state_number / full_strength);
        emit change_state();
    }
    if (state_now >= state_number)
        return;
    ...
    for (int i = 0; i < parent->bullets[ypos].count(); i++)
        if (!(parent->bullets[ypos][i]->is_dead) &&
            abs(parent->bullets[ypos][i]->x() - x() - 50) < 20)
        {
            if (x() < 1000)
    ...
    if (xpos >= 0 && xpos < 9 && ypos >= 0 && ypos < 5 && parent->plants[ypos][xpos] != nullptr &&
        !if_stop)
...
ordinary::ordinary(int loc, GameWidget *parent)
    : Zombie(loc, 10, 1, 2, 1, 2, ordinary_formal, ordinary_eat, parent)
{
    // 普通僵尸构造
}

barricade::barricade(int loc, GameWidget *parent)
    : Zombie(loc, 20, 1, 2, 1, 3, barricade_formal, barricade_eat, parent)
{
    // 路障僵尸构造
}

drums::drums(int loc, GameWidget *parent)
    : Zombie(loc, 30, 1, 2, 1, 3, drums_formal, drums_eat, parent)
{
    // 铁通僵尸构造
}
