...
Zombiethread::Zombiethread(GameWidget* ptr):parent(ptr){            //僵尸生成的控制器，实质上是QTimer
...
void Zombiethread::prepare_for_next(){   //timeout时生成一个僵尸，同时根据时序表改变自身interval，再次启动
    if(zombieid.count() <= 1)
        return;
...
void Mainthread::run(){                                     //主要线程的执行
    while(parent->result == -1){                            //游戏尚未结束时进入循环
        msleep(10);
